...
public class TeleportStraight : MonoBehaviour
{
    ...
    // 최초의 텔레포트 UI 크기
    Vector3 originalScale = Vector3.one * 0.02f;
    ...
    void Update()
    {
        ...
        else if (ARAVRInput.Get(ARAVRInput.Button.One, ARAVRInput.Controller.LTouch))
        {
            ...
            if (Physics.Raycast(ray, out hitInfo, 200, layer))
            {
                ...
                // 텔레포트 UI가 위로 향하도록 방향 설정
                teleportCircleUI.forward = hitInfo.normal;
                teleportCircleUI.localScale = originalScale * Mathf.Max(1, hitInfo.distance);

            }
            else
            {
                // Ray 충돌이 발생하지 않으면 선이 Ray방향으로 그려지도록 쳐라
                lr.SetPosition(0, ray.origin);
